...
Having the efficient scripting algorithms in cartographic data processing of geospatial datasets can bring significant speed improvements to mapping process \cite{CHEN2022105362,ORTOLANO201856, Lemenkova202203,OMRAN2016140}. Examples of such applications include applying the modules of GMT for specific cartographic tasks of data processing and visualisation. Effective processing of geospatial data by scripts is especially actual for large datasets which are the case for recent advances in Earth observation with global coverage \cite{LEI2020202,XIA2018245,NATIVI20151,WAGEMANN2021104916}. Console-based data processing using scripts enables to increase the accuracy and speed of data processing in cartographic analysis and plotting. Therefore, using advanced geospatial software and tools is also of major importance in many GIS-related applications that includes geospatial data processing, such as machine learning applications in GIS \cite{1607434,ARABAMERI2021100848,ZUO2021105111,SUN201926}, data retrieval, exploration and processing \cite{BUCK2022121,Lemenkova202135,1369823}, spatial data formatting and compression \cite{, , , }, geographic data modelling and analysis \cite{MA2021103858,Lemenkova202205,TAMIMINIA2020152}. 

Processing geographic data is important with regard to the variety of existing data formats. The two major geographic data formats, raster and vector types, are often used in a combination in a mapping workflow. 

Raster data and images are often derived from the remote sensing (RS) sources as satellite images. Examples of raster geographic data include high-resolution satellite imagery: Sentinel-2A, Landsat TM/ETM+ by ESA, aerial Google Earth or the commercial geospatial data providers  operating of large-scale aerial data. More data sources include hyperspectral imagery, InSAR, LiDAR, which is used for estimating slope steepness \cite{, , }, Important geospatial data source grids is given by the SRTM DEM for topographic mapping. Using spectral bands of raster data is often used for land cover mapping and landscape analysis \cite{, , }. 

Another source of raster data includes scanned raster images in bitmap formats (jpg, bmp, tiff). Used as georeferenced raster layers in GIS, archived historical raster maps can be used as data sources for estimating changes level to evaluate landscape dynamics and serve as auxiliary source for geologic analysis and mineral resource management. Technically, processing raster  data is based on the analysis of pixels on the image and, if necessary, steps of vectorisation. In many cases, the RS data can be processed using the ML. 

Vector data sources include, for instance, the geological vectorised objects available as georeferenced layers in specific GIS formats (\emph{e.g.}, .shp format of the ArcGIS/QGIS).

This study is focused on the processing of the geospatial data aimed at cartographic data visualisation and mapping correlations between the geological, topographic and geophysical setting in Bolivia, Central Andes (Figure~\ref{fig1}). 
...
Geologic maps (Figure~\ref{fig2} and Figure~\ref{fig3}) reflect current state of the solid earth processes in Andes region. The metamorphic rocks and outcrops of various geologic epochs experienced solid-state mineral transformations due to changes in pressure and temperature in crust and mantle of the Earth, and record geological periods, heating, seismic processes and cooling that reflect the tectonic environments in which they formed \cite{Holder}. Complex geological history of Andes is well reflected in the geology and petrographic setting of the Bolivian Andes, as well as the origin, distribution, and geochemistry of selected minerals \cite{Tapia}. 
...
\begin{figure}[H]\centering
	...
	\caption{Seismicity in Bolivia and the Central Andes. Visualization: GMT. Source: author.\label{fig7}}
\end{figure}   
